...
SWEP.PrintName = "Lance of Vitality"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A demonic spear that accurately pierces an enemy's weak spot. Increases Max HP by 25. Every Rakukaja on the wielder buffs crit damage by 2%. \n[STR 6] Crit damage buff 3% instead [DEX 6] Technique: 15 [CHR 6] Bonus Crit Chance 10%"
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            local tech = 10
            if playerDex >= 6 then tech = 15 end
            ...
            local critBonus = 0
            if playerChr >= 6 then critBonus = critBonus + 10 end
            ...
            targetEffects = HandleCrit(targetEffects["ply"],
                                       targetEffects["target"], targetEffects)
            if targetEffects["state"] == 'crit' and userBuffsTable["Rakukaja"] then
                local playerStr = tonumber(
                                      targetEffects["ply"]:GetNWInt("TBCSTR", 0))
                if playerStr >= 6 then
                    targetEffects["baseDamage"] =
                        targetEffects["baseDamage"] *
                            (1 + (0.03 * userBuffsTable["Rakukaja"]["stacks"]))
                else
                    targetEffects["baseDamage"] =
                        targetEffects["baseDamage"] *
                            (1 + (0.02 * userBuffsTable["Rakukaja"]["stacks"]))
                end
            end
    ...
    if SERVER then
        timer.Create(ply:Name() .. self.PrintName, 0.2, 1, function()
            ...
            if currentHP >= maxHP then
                ply:SetNWInt("TBCHP", maxHP + 25)
            end
            ply:SetNWInt("TBCMAXHP", maxHP + 25)
        end)
    end
end
...
function SWEP:DropFunction()
    if SERVER and IsValid(self.Owner) then
        ...
        if currentHP >= maxHP then
            self.Owner:SetNWInt("TBCHP", maxHP - 25)
        end
        self.Owner:SetNWInt("TBCMAXHP", maxHP - 25)
    end
end
