...
SWEP.PrintName = "Bladed Boomerang"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A dangerous weapon that requires another level of mastery to avoid self-harm while rewarding surprise attacks. Reduce's wielder's HP by 5 upon use. Hitting an enemy with HP 50% or higher deals bonus (STR + CHR)% crit damage. \n[DEX 3] Self-damage penalty removed."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if playerDex >= 3 then hpCost = 0 end

            if attackerHP >= hpCost then
                ply:SetNWInt("TBCHP", attackerHP - hpCost)
            else
                ply:ChatPrint("Not enough HP to use this.")
                ply:LagCompensation(false)
                return
            end

            if not self:AbilityRollNumber(10, target) then
                ply:LagCompensation(false)
                return
            end
            ...
            if targetEffects["state"] == "crit" and currentHP >= maxHP * 0.5 then
