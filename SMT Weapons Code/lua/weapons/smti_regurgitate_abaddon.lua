...
SWEP.PrintName = "Regurgitate (Abaddon)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Drains 30 HP from an enemy. Gain +1 Rakukaja. If Rakukaja is +4, decrease target's current MP by 24 instead."
SWEP.Instructions = ""
        ...
        if SERVER and IsValid(target) then
            ...
            targetEffects["message"] =
                targetEffects["target"]:Name() .. " has " ..
                    targetEffects["baseDamage"] .. " HP drained by " ..
                    targetEffects["ply"]:Name() .. "!"

            local currentHP = targetEffects["ply"]:GetNWInt("TBCHP", 100)
            local maxHP = targetEffects["ply"]:GetNWInt("TBCMAXHP", 100)
            local newHP = math.min(currentHP + targetEffects["baseDamage"],
                                   maxHP)

            targetEffects["ply"]:SetNWInt("TBCHP", newHP)
            ...
            if targetEffects["userBuffsTable"]["Rakukaja"] then
                if targetEffects["userBuffsTable"]["Rakukaja"].stacks < 4 then

                    targetEffects["userBuffsTable"]["Rakukaja"].stacks =
                        math.min(targetEffects["userBuffsTable"]["Rakukaja"]
                                     .stacks + 1, 4)
                    ...
                    local currentMP = targetEffects["target"]:GetNWInt("TBCMP",
                                                                       100)
                    local maxMP = targetEffects["target"]:GetNWInt("TBCMAXMP",
                                                                   100)
                    local newMP
                    ...
                    targetEffects["message"] =
                        targetEffects["target"]:Name() .. " has lost 24 MP!"
