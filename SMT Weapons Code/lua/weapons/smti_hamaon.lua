...
SWEP.PrintName = "Hamaon"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "80 light damage to a single target. The following conditions trigger instakill based on percent value and add onto each other: Weak to Light: 1% Marked with Judgement : 5% [CHR 6] Marked with Judgement chance is 6% instead."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if targetEffects["targetDebuffsTable"]["Judgement"] then
                targetEffects["killChance"] = targetEffects["killChance"] + 5
            end
            ...
            if math.random(1, 100) <= targetEffects["killChance"] then
                HandleInstakill(targetEffects["ply"], targetEffects["target"],
                                targetEffects)
            end
