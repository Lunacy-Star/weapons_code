...
SWEP.PrintName = "Fortune's Kiss"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Heals all party members for 25% of their max HP and applies Lydia and Tarukaja +1 on all party members. Initial heal amount becomes 15% of max HP if more than one Pixie is in the party."
SWEP.Instructions = ""
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if inAFight then
                ...
                local pixieCounter = 0
                for _, player in ipairs(fight[playerSide]) do
                    if IsValid(player) then
                        table.insert(playersInFight, player)
                        if player:Team() == "Pixie" then
                            pixieCounter = pixieCounter + 1
                        end
                    end
                end

                local percentHeal = 0.25
                if pixieCounter >= 2 then percentHeal = 0.15 end
                ...
                for _, target in ipairs(playersInFight) do
                    ...
                    if currentHP > 0 then
                        ...
                        targetEffects["percentHeal"] = percentHeal
                        ...
                        AssignStat(target, "Tarukaja",
                                   targetBuffsTable["Tarukaja"], "buffs")

                        targetEffects["buff"] = "Tarukaja"

                        HandleStatus(target, targetBuffsTable, "reactionBuff",
                                     "buff", targetEffects)

                        targetBuffsTable["Lydia"] = {
                            stacks = 1,
                            type = "turnRegen",
                            duration = 3
                        }

                        AssignStat(target, "Lydia", targetBuffsTable["Lydia"],
                                   "buffs")

                        targetEffects["buff"] = "Lydia"
                        ...
                        local message = target:Name() .. " received " ..
                                            ...
                                            " and now has Lydia and Tarukaja!"
                ...
                for _, target in ipairs(playersInFight) do
                    if IsValid(target) then -- Check if the player is valid
                        ...
                        if currentHP > 0 then
                            ...
                            AssignStat(target, "Lydia",
                                       targetBuffsTable["Lydia"], "buffs")
