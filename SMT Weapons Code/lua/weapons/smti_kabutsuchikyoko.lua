...
SWEP.PrintName = "Kabutsuchi Kyoko"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A manly weapon with a head that has been toughened to an incredible degree. A single swing is all it takes to win, there is nothing this weapon cannot break. Every Rakukaja on the wielder increases the weapon's Technique by +4. \n[STR 6] Tarukaja also triggers the effect."
SWEP.Instructions = ""
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if userBuffsTable["Rakukaja"] then
                tech = tech + (4 * userBuffsTable["Rakukaja"]["stacks"])
            end

            if playerStr >= 6 and userBuffsTable["Tarukaja"] then
                tech = tech + (4 * userBuffsTable["Tarukaja"]["stacks"])
            end
