...
SWEP.PrintName = "Quick Life Drain"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose = "Deals 15 Almighty damage on target and heals for 15 HP."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            local healerHP = ply:GetNWInt("TBCHP", 100)
            local healerMaxHP = ply:GetNWInt("TBCMAXHP", 100)
            healerHP = math.min(healerHP + self.HealAmount, healerMaxHP)
            ply:SetNWInt("TBCHP", healerHP)
            ply:ChatPrint("You recover " .. self.HealAmount ..
                              " HP from Quick Life Drain!")
