...
SWEP.PrintName = "Roaring Engine"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 10 Nuke damage to all enemies. If one of the enemies she is targeting is inflicted with an elemental ailment, deal 20 Nuke damage to that target instead."
SWEP.Instructions = ""
    ...
    if SERVER then
        ...
        if next(targetedTargets) == nil then
            ...
            for _, ent in pairs(targetedTargets) do
                ...
                for status, properties in pairs(
                                              targetEffects["targetDebuffsTable"]) do
                    if table.HasValue(Affinities["Elements"], status) then
