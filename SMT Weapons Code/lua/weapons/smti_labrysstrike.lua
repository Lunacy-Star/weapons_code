...
SWEP.PrintName = "Labrys Strike"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "[Physical] [Phase 2: 1500 HP or lower]\nDeals 1-3 hits to multiple foes, each dealing 50 physical damage."
SWEP.Instructions = ""
    ...
    if tr.Hit then
        if SERVER then
            ...
            if currentHP > self.PhaseThreshold then
                ply:ChatPrint("Labrys Strike is not available yet.")
                ply:LagCompensation(false)
                return
            end
            ...
            if next(targetedTargets) == nil then
                ...
                local loopIterations = math.random(1, 3)
