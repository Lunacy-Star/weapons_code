...
SWEP.PrintName = "Ebony Tonfa"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A set of ebony tonfa designed to allow simultaneous attack and defense moves. \n[STR 6] Damage: 45. \n[DEX 6] Technique 25. \n[CHR 6] Bonus Crit Chance 15%"
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if playerDex >= 6 then tech = 25 end
            ...
            if playerStr >= 6 then targetEffects["baseDamage"] = 45 end
            ...
            local critBonus = 0
            if playerChr >= 6 then critBonus = critBonus + 15 end
...
function SWEP:Equip(ply)
    if SERVER then
        timer.Create(ply:Name() .. self.PrintName, 0.2, 1, function()
            local playerLuck = ply:GetNWInt("TBCLuck", 10)
            ply:SetNWInt("TBCLuck", playerLuck + 1)
        end)
    end
end
...
function SWEP:DropFunction()
    if SERVER and IsValid(self.Owner) then
        ...
        self.Owner:SetNWInt("TBCLuck", playerLuck - 1)
    end
end
