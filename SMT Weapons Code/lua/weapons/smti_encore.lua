...
SWEP.PrintName = "Encore"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Increase the effectiveness on all Kaja buffs on Rise's party for two turns."
SWEP.Instructions = ""
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            for _, target in ipairs(playersInFight) do
                ...
                if target ~= ply then
                    if buffsTable["Encore"] then
                        buffsTable["Encore"].duration = 3
                    else
                        buffsTable["Encore"] = {stacks = 1, duration = 3}
                    end
                else
                    if buffsTable["Encore"] then
                        buffsTable["Encore"].duration = 4
                    else
                        buffsTable["Encore"] = {stacks = 1, duration = 4}
                    end
                end

                AssignStat(target, "Encore", buffsTable["Encore"], "buffs")

                local message = target:Name() .. " received Encore from " ..
                                    ply:Name() .. "!"
    ...
    if ply:IsPlayer() then
        ...
        if SERVER and IsValid(target) then
            ...
            for _, player in ipairs(playersInFight) do
                ...
                AssignStat(player, "Encore", buffsTable["Encore"], "buffs")

                local message = player:Name() .. " received Encore from " ..
                                    ply:Name() .. "!"
