...
SWEP.PrintName = "Iron Pipe Shotgun"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A shotgun made from an iron pipe. If wielder is equipped with Iron Pipe, Iron Pipe's effect no longer reduces Technique."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            local tech = -5
            local pipeWeapon = self.Owner:GetWeapon("smti_ironpipe")
            if IsValid(pipeWeapon) then tech = 0 end
