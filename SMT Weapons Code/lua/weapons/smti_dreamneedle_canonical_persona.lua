...
SWEP.PrintName = "Dream Needle (Persona)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 45 gun damage on a single target. 26% chance to inflict Sleep on hit. \n[STR 3 + DEX 4] HP Cost reduced to 10 HP [CHR 5] 34% chance to inflict Sleep on hit."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if playerStr >= 3 and playerDex >= 4 then hpCost = 10 end
            ...
            targetEffects["ailmentChance"] = 26
            local playerChr = tonumber(ply:GetNWInt("TBCCHR", 0))
            if playerChr >= 5 then
                targetEffects["ailmentChance"] = 34
            end
            ...
            if math.random(1, 100) <= targetEffects["ailmentChance"] then
                ...
                AssignStat(targetEffects["target"], "Sleep",
                           targetDebuffsTable["Sleep"], "debuffs")

                self:AnnounceMessage(targetEffects["target"]:Name() ..
                                         " is now Asleep!")
                HandleStatus(targetEffects["ply"], userBuffsTable,
                             "ailmentReaction", "Sleep", targetEffects)
            end
