...
SWEP.PrintName = "Kozan of the South"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A set of tonfa handed to only those who have trained at the South Mountain, one of five mountain training grounds. Those who wield them are energized to draw out the potential of their body and mind. Deal additional damage based on 10% of Max HP. \n[STR 6] Additional damage from 12% of Max HP [DEX 6] Technique 20 [CHR 6] Base damage increased to 15."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if playerDex >= 6 then tech = 20 end
            ...
            if playerChr >= 6 then targetEffects["baseDamage"] = 15 end
            ...
            local maxMPPercentage = 0.10
            local playerStr = tonumber(ply:GetNWInt("TBCSTR", 0))
            if playerStr >= 6 then maxMPPercentage = 0.12 end

            targetEffects["baseDamage"] =
                targetEffects["baseDamage"] + (maxMP * maxMPPercentage)
