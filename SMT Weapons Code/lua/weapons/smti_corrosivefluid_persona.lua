...
SWEP.PrintName = "Corrosive Fluid (Persona)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 40 Ruin damage to a single target with a 40% chance of inflicting Poison. Heals 10 HP upon use."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            local casterHP = ply:GetNWInt("TBCHP", 100)
            local casterMaxHP = ply:GetNWInt("TBCMAXHP", 100)
            ply:SetNWInt("TBCHP", math.min(casterHP + 10, casterMaxHP))
            self:AnnounceMessage(ply:Name() .. " recovers 10 HP!")
            ...
            targetEffects["ailmentChance"] = 40
            ...
            if math.random(1, 100) <= targetEffects["ailmentChance"] then
                ...
                AssignStat(targetEffects["target"], "Poison",
                           targetDebuffsTable["Poison"], "debuffs")
