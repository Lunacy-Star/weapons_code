...
SWEP.PrintName = "Tarunda (Persona)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Decreases Tarukaja count by 1. Every Tarunda decreases damage dealt by 0.2x. Minimum Tarukaja count = -4."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"] then
                targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"].stacks =
                    math.min(targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"]
                                 .stacks + 1, 4)
            else
                targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"] = {stacks = 1}
            end

            AssignStat(target, "Tarunda_HuaPo",
                       targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"],
                       "debuffs")
...
function SWEP:SecondaryAttack()
    ...
    if SERVER and IsValid(target) then
        ...
        if targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"] then
            targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"].stacks = math.min(
                                                                        targetEffects["targetDebuffsTable"]["Tarunda_HuaPo"]
                                                                            .stacks +
                                                                            1, 4)
        else
        ...
        targetEffects["debuff"] = "Tarunda_HuaPo"
