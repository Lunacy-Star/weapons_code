...
SWEP.PrintName = "Provoke (Persona)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Tarukaja +2 on all enemies, Rakunda +2 on all enemies. 30% chance they are inflicted with Rage."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if true then
        ...
        if SERVER and IsValid(target) then
            ...
            local rageChance = math.ceil(playerLuck / 2) + 30
            ...
            for _, player in ipairs(playersInFight) do
                ...
                if buffsTable["Tarukaja"] then
                    buffsTable["Tarukaja"].stacks = math.min(
                                                        buffsTable["Tarukaja"]
                                                            .stacks + 2, 4)
                else
                    buffsTable["Tarukaja"] = {stacks = 2}
                end

                AssignStat(player, "Tarukaja", buffsTable["Tarukaja"], "buffs")
                ...
                HandleStatus(player, buffsTable, "reactionDebuff", "debuff",
                             targetEffects)

                local message = player:Name() .. " is provoked by " ..
                                    ply:Name() .. "! They now have " ..
                                    targetEffects["targetDebuffsTable"]["Rakunda"]
                                        .stacks .. " Rakunda but also have " ..
                                    buffsTable["Tarukaja"].stacks ..
                                    " Tarukaja!"
                ...
                if math.random(1, 100) <= rageChance then
                    ...
                    targetDebuffsTable["Rage"] = {
                        stacks = 1,
                        wearOff = "turnWearOff",
                        duration = 4
                    }

                    AssignStat(player, "Rage", targetDebuffsTable["Rage"],
                               "debuffs")

                    self:AnnounceMessage(player:Name() .. " is now Enraged!")
