...
SWEP.PrintName = "Mana Eater"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A weapon developed as an attempt to recreate the Deathbringer Axe with human hands, following a detailed analysis of its structure. It is a prototype created by a certain defense contractor, and offers powers unimaginable in a commercial product. Reduce Max HP by 40 and increases Max SP by 20. Cannot critically strike with this weapon. Every attack dealt with this weapon gives the wielder SP based on CHR value."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            local playerChr = tonumber(
                                  targetEffects["ply"]:GetNWInt("TBCCHR", 0))
            local currentMP = targetEffects["ply"]:GetNWInt("TBCMP", 100)
            local maxMP = targetEffects["ply"]:GetNWInt("TBCMAXMP", 100)

            local bonusChr = 2
            if playerChr > 0 then bonusChr = bonusChr + playerChr end

            local newMP = math.min(currentMP + bonusChr, maxMP)
            targetEffects["ply"]:SetNWInt("TBCMP", newMP)
            targetEffects["ply"]:ChatPrint("You recover " .. playerChr .. " SP!")
    ...
    if SERVER then
        timer.Create(ply:Name() .. self.PrintName, 0.2, 1, function()
            ...
            if currentHP >= maxHP then
                ply:SetNWInt("TBCHP", maxHP - 40)
            end
            ply:SetNWInt("TBCMAXHP", maxHP - 40)

            local currentMP = ply:GetNWInt("TBCMP", 100)
            local maxMP = ply:GetNWInt("TBCMAXMP", 100)
            if currentMP >= maxMP then
                ply:SetNWInt("TBCMP", maxMP + 20)
            end
            ply:SetNWInt("TBCMAXMP", maxMP + 20)
        end)
    end
end
...
function SWEP:DropFunction()
    if SERVER and IsValid(self.Owner) then
        ...
        if currentHP >= maxHP then
            self.Owner:SetNWInt("TBCHP", maxHP + 40)
        end
        self.Owner:SetNWInt("TBCMAXHP", maxHP + 40)

        local currentMP = self.Owner:GetNWInt("TBCMP", 100)
        local maxMP = self.Owner:GetNWInt("TBCMAXMP", 100)
        if currentMP >= maxMP then
            self.Owner:SetNWInt("TBCMP", maxMP - 20)
        end
        self.Owner:SetNWInt("TBCMAXMP", maxMP - 20)
    end
end
