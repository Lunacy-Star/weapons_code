...
SWEP.PrintName = "Orgia Mode"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Aigis deals 100 Almighty damage to all enemies. Inflict self with 1 Rakunda and 1 Sukunda after the attack."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if not PlayerCanUseHPSkills(ply, hpCost, userBuffsTable,
        ...
        if next(targetedTargets) == nil then
            ...
            for _, ent in pairs(targetedTargets) do
                ...
                if targetEffects["userDebuffsTable"]["Rakunda"] then
                    targetEffects["userDebuffsTable"]["Rakunda"].stacks =
                        math.min(targetEffects["userDebuffsTable"]["Rakunda"]
                                     .stacks + 1, 4)
                else
                    targetEffects["userDebuffsTable"]["Rakunda"] = {stacks = 1}
                end

                AssignStat(targetEffects["target"], "Rakunda",
                           targetEffects["userDebuffsTable"]["Rakunda"],
                           "debuffs")

                targetEffects["debuff"] = "Rakunda"

                HandleStatus(targetEffects["target"],
                             GetAllStats(targetEffects["target"], "buffs"),
                ...
                if targetEffects["userDebuffsTable"]["Sukunda"] then
                    targetEffects["userDebuffsTable"]["Sukunda"].stacks =
                        math.min(targetEffects["userDebuffsTable"]["Sukunda"]
                                     .stacks + 1, 4)
                else
                    targetEffects["userDebuffsTable"]["Sukunda"] = {stacks = 1}
                end

                AssignStat(targetEffects["target"], "Sukunda",
                           targetEffects["userDebuffsTable"]["Sukunda"],
                           "debuffs")
