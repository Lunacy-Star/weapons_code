...
SWEP.PrintName = "Caladbolg"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A heavy lance with a name that translate into 'Hard Scabbard' that was wielded by Cu Chulainn's friend Fergus. A single blow with this weapon is said to be able to split a hill down the middle. Every Sukukaja on the wielder buffs crit damage and bonus crit chance by 3%."
SWEP.Instructions = ""
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            targetEffects = HandleCrit(targetEffects["ply"],
                                       targetEffects["target"], targetEffects)
            if targetEffects["state"] == 'crit' and userBuffsTable["Sukukaja"] then
                targetEffects["baseDamage"] =
                    targetEffects["baseDamage"] *
                        (1 + (0.03 * userBuffsTable["Sukukaja"]["stacks"]))
            end
