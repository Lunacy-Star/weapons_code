...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if currentSelfHP > maxSelfHP * self.PhaseThreshold then
            ply:ChatPrint("Fire Breath is not available yet.")
            ply:LagCompensation(false)
            return
        end
        ...
        if next(targetedTargets) == nil then
            ...
            local loopIterations = math.random(2, 4)
            ...
            for i = 1, loopIterations do
                ...
                if math.random(1, 100) <= targetEffects["ailmentChance"] then
                    ...
                    self:AnnounceMessage(
                        targetEffects["target"]:Name() .. " is now Burning!")
