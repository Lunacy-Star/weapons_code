...
SWEP.PrintName = "Teardrop (Persona)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "20 martial art damage on a single target. Lowers target's Rakukaja by -1. [STR 5] HP Cost reduced to 10 HP. [DEX 5] Technique 25."
SWEP.Instructions = ""
SWEP.Category = "Persona Physical Skills"
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if targetDebuffsTable["Rakunda"] then
                targetDebuffsTable["Rakunda"].stacks =
                    math.min(targetDebuffsTable["Rakunda"].stacks + 1, 4)
            else
                targetDebuffsTable["Rakunda"] = {stacks = 1}
            end

            AssignStat(targetEffects["target"], "Rakunda",
                       targetDebuffsTable["Rakunda"], "debuffs")

            local rakundaEffects = {debuff = "Rakunda"}

            HandleStatus(targetEffects["target"],
                         GetAllStats(targetEffects["target"], "buffs"),
                         "reactionDebuff", "debuff", rakundaEffects)

            self:AnnounceMessage(targetEffects["target"]:Name() ..
                                     " received Rakunda! They now have " ..
                                     targetDebuffsTable["Rakunda"].stacks ..
                                     " stacks!")
