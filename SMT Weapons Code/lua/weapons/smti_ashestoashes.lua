...
SWEP.PrintName = "Ashes to Ashes"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 45 Almighty damage on a single target. If the target has an ailment on them, deal 80 Almighty damage instead. \n[STR 5] Damage on ailment-induced target increases to 85. \n[DEX 5] Technique penalty removed. \n[CHR 5] MP cost reduced to 26."
SWEP.Instructions = ""
        ...
        if SERVER and IsValid(target) then
            ...
            for status, properties in pairs(targetEffects["targetDebuffsTable"]) do
                if table.HasValue(Ailments_Statuses["Ailments"], status) then
                    targetEffects["baseDamage"] = 80

                    if playerStr >= 5 then
                        targetEffects["baseDamage"] = 85
                    end
                    break
                end
            end
