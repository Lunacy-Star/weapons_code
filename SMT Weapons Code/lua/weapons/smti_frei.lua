...
SWEP.Contact = ""
SWEP.Purpose =
    "40 nuke damage to a single target. If hitting an enemy inflicted by Burn/Freeze/Shock/Confusion, 20% bonus crit chance. \n[CHR 4] MP Cost reduced to 4 MP. \n[DEX 4] Bonus crit chance effect is 25% instead."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if playerChr >= 4 then mpCost = 4 end
            ...
            if not self:AbilityRollNumber(15, target) then
                ply:LagCompensation(false)
                return
            end
            ...
            if targetEffects["targetDebuffsTable"]["Burn"] or
                ...
                targetEffects["critChance"] = targetEffects["critChance"] + 20
                if playerDex >= 4 then
                    targetEffects["critChance"] =
