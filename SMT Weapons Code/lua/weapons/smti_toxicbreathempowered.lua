...
SWEP.PrintName = "Toxic Breath (Empowered)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "[Dark] [Phase 3: 40% of Max HP or lower]\nDeals 80 Dark attack to all enemies. 30% chance of inflicting Poison. Poison inflict chance increases to 100% if enemy is weak to Dark. For every target that blocks or repels the attack, +1 Tarukaja."
SWEP.Instructions = ""
...
SWEP.PoisonChance = 30
SWEP.PoisonChanceWeak = 100
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        local currentSelfHP = ply:GetNWInt("TBCHP", 0)
        local maxSelfHP = ply:GetNWInt("TBCMAXHP", 1)
        if currentSelfHP > maxSelfHP * self.PhaseThreshold then
            ply:ChatPrint("Toxic Breath (Empowered) is not available yet.")
            ply:LagCompensation(false)
            return
        end
        ...
        local blockedOrRepelledCount = 0
        ...
        if next(targetedTargets) == nil then
            ...
            for _, ent in pairs(targetedTargets) do
                ...
                -- Pierce: ignores Resist entirely, but still tracks Block/Repel to reward Tarukaja below.
                ...
                if targetEffects["state"] == 'repel' then
                    ...
                    blockedOrRepelledCount = blockedOrRepelledCount + 1
                end
                ...
                targetEffects["ailmentChance"] =
                    (targetEffects["state"] == "weak") and self.PoisonChanceWeak or
                        self.PoisonChance
                ...
                targetEffects = HandleBlock(targetEffects["ply"],
                                            targetEffects["target"],
                                            targetEffects)
                if targetEffects["state"] == 'block' then
                    blockedOrRepelledCount = blockedOrRepelledCount + 1
                end
                ...
                if math.random(1, 100) <= targetEffects["ailmentChance"] then
                    ...
                    AssignStat(targetEffects["target"], "Poison",
                               debuffsTable["Poison"], "debuffs")

                    self:AnnounceMessage(
                        targetEffects["target"]:Name() .. " is now Poisoned!")

                    HandleStatus(targetEffects["ply"], userBuffsTable,
                                 "ailmentReaction", "Poison", targetEffects)
                end
        ...
        if blockedOrRepelledCount > 0 then
            local finalUserBuffsTable = GetAllStats(ply, "buffs")
            local currentStacks = (finalUserBuffsTable["Tarukaja"] and
                                       finalUserBuffsTable["Tarukaja"].stacks) or
                                      0
            local newStacks =
                math.min(currentStacks + blockedOrRepelledCount, 4)

            AssignStat(ply, "Tarukaja", {stacks = newStacks}, "buffs")

            self:AnnounceMessage(
                ply:Name() .. " gains Tarukaja from Toxic Breath (Empowered)! (" ..
                    newStacks .. " stacks)"
            )
        end
