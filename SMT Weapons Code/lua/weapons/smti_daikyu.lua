...
SWEP.PrintName = "Daikyū"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A longbow often used by Kyūdō practitioners. They were once an essential weapon during Feudal Japan. Every Tarukaja or Rakukaja increases the Technique of this weapon by 3."
SWEP.Instructions = ""
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if userBuffsTable["Tarukaja"] then
                tech = tech + (3 * userBuffsTable["Tarukaja"]["stacks"])
            end
            if userBuffsTable["Rakukaja"] then
                tech = tech + (3 * userBuffsTable["Rakukaja"]["stacks"])
            end
