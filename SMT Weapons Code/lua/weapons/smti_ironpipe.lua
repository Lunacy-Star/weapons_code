...
SWEP.PrintName = "Iron Pipe"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "An iron pipe in construction work, that when wielded gives off a violent mood. Every Rakukaja on the wielder buffs the damage by 5 but reduces Technique by -5."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            local pipeWeapon = self.Owner:GetWeapon("smti_ironpipeshotgun")
            if IsValid(pipeWeapon) then
                if userBuffsTable["Rakukaja"] then
                    tech = tech - (5 * userBuffsTable["Rakukaja"]["stacks"])
                end
            end
            ...
            if userBuffsTable["Rakukaja"] then
                targetEffects["baseDamage"] =
                    targetEffects["baseDamage"] +
                        (5 * userBuffsTable["Rakukaja"]["stacks"])
            end
