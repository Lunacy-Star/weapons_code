...
SWEP.PrintName = "Skull Cracker (Persona)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 60 blunt damage on a single target. 25% chance to inflict Paralyze on hit. \n[STR 6] HP Cost reduced to 12 HP. \n[CHR 6] 30% chance to inflict Panic on hit"
SWEP.Instructions = ""
        ...
        if SERVER and IsValid(target) then
            ...
            if math.random(1, 100) <= targetEffects["ailmentChance"] then
                ...
                AssignStat(targetEffects["target"], "Paralysis",
                           debuffsTable["Paralysis"], "debuffs")
            ...
            if playerChr >= 6 then
                local panicChance = 30
                panicChance = math.ceil(playerLuck / 2) + panicChance

                panicChance = panicChance +
                                  (HandleStatus(targetEffects["ply"],
                                                userBuffsTable,
                                                "increaseAilmentChance",
                                                panicChance, targetEffects) -
                                      HandleStatus(targetEffects["ply"],
                                                   userDebuffsTable,
                                                   "decreaseAilmentChance",
                                                   panicChance, targetEffects)) -
