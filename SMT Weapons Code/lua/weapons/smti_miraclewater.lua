...
SWEP.PrintName = "Miracle Water"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "75 HP restored for Nahobino's party. Cures all status ailments on allies with HP less than 5 + CHR% of their Max HP."
SWEP.Instructions = ""
                    ...
                    if IsValid(target) then -- Check if the player is valid
                        ...
                        if currentHP > 0 then
                            ...
                            local playerChr = tonumber(
                                                  target:GetNWInt("TBCCHR", 0))
                            local percentHP = 5 + playerChr
                            percentHP = (percentHP * maxHP) / 100

                            if currentHP <= percentHP then
                                local debuffsTable = GetAllStats(target,
                                                                 "debuffs")
                                ...
                                message =
                                    target:Name() .. " received " ..
                                        targetEffects["baseDamage"] ..
                                        " healing from " .. ply:Name() ..
                                        "! Furthermore, they had their ailments cleansed."
                            end
