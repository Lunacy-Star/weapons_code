...
SWEP.PrintName = "Bead Chain"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A chain of beads that, when broken, causes an explosive recovery of health and mana. Ammo Stack = 1. Effect: Heals party by 65% of their Max HP and recovers 50% of their Max MP."
SWEP.Instructions = ""
                ...
                for _, target in ipairs(playersInFight) do
                    if IsValid(target) then -- Check if the player is valid
                        ...
                        if currentHP > 0 then
                            ...
                            targetEffects["percentHeal"] = 0.65
                            ...
                            local mpToRegen = maxMP * 0.50

                            local newMP = math.min(currentMP + mpToRegen, maxMP)
                    ...
                    if IsValid(target) and CheckIfValidTBCEntity(target) then -- Check if the player is valid
                        ...
                        if targetEngageSWEP.FightId == self.FightId then
                            ...
                            if currentHP > 0 then
                                ...
                                local newMP =
                                    math.min(currentMP + mpToRegen, maxMP)
