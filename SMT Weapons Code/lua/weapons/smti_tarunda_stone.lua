...
SWEP.PrintName = "Tarunda Stone"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A stone that increases -nda debuff properties of a user. Decreases Tarukaja count by -1. Every Tarunda decreases damage dealt by 0.1x. Minimum Tarukaja count = -4."
SWEP.Instructions = ""
...
function SWEP:SecondaryAttack()
    ...
    if SERVER and IsValid(target) then
        ...
        if targetEffects["targetDebuffsTable"]["Tarunda"] then
            targetEffects["targetDebuffsTable"]["Tarunda"].stacks = math.min(
                                                                        targetEffects["targetDebuffsTable"]["Tarunda"]
                                                                            .stacks +
                                                                            1, 4)
        else
        ...
        local message =
            target:Name() .. " received Tarunda from " .. ply:Name() ..
                "! They now have " ..
                targetEffects["targetDebuffsTable"]["Tarunda"].stacks ..
                " stacks!"
