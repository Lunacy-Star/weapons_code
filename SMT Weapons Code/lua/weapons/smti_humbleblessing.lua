...
SWEP.PrintName = "Humble Blessing"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "MP Cost: 14.\nHeal based on 20% of all allies' max base HP. Heal 60 HP instead if Max Base HP <= 300. \n[CHR 4] Heal 25% of max base HP or 75 HP instead. In addition, if there is only one Angel present in a party, all allies aside from Angel recover 12 MP."
SWEP.Instructions = ""
...
-- If Angel is the only Angel present on the healed side, everyone else on
-- that side recovers 12 MP.
local function ApplySoleAngelBonus(ply, playersInFight, weapon)
    local angelCount = 0
    for _, member in ipairs(playersInFight) do
        if IsValid(member) then
            local charId = member:GetNWString("AssignedCharacter")
            local charData = CHARACTERS.List[charId]
            if charData and charData.variantGroup == "angel" then
                angelCount = angelCount + 1
            end
        end
    end

    if angelCount ~= 1 then return end

    for _, member in ipairs(playersInFight) do
        if IsValid(member) and member ~= ply then
            local currentMP = member:GetNWInt("TBCMP", 100)
            local maxMP = member:GetNWInt("TBCMAXMP", 100)
            local newMP = math.min(currentMP + 12, maxMP)

            member:SetNWInt("TBCMP", newMP)
        end
    end

    if IsValid(weapon) then
        weapon:AnnounceMessage(ply:Name() .. "'s Humble Blessing restores 12 MP to the party!")
    end
end
            ...
            if inAFight then
                ...
                for _, target in ipairs(playersInFight) do
                    ...
                    if currentHP > 0 then
                        ...
                        local newHP = math.min(currentHP +
                                                   targetEffects["baseDamage"],
                                               maxHP)
                ...
                ApplySoleAngelBonus(ply, playersInFight, self)
