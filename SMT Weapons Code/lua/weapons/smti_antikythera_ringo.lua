...
SWEP.PrintName = "Antikythera (Ringo)"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Ringo's COMP doubles as a weapon, capable of transforming into a weapon dealing 60 Blade-type damage with +10 Technique.\n[STR 4] 65 Damage \n[DEX 4] 15+ Technique \n[CHR 4] +5 Luck and 10% Bonus Crit Chance."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if playerDex >= 4 then totalTech = 15 end

            if not self:AbilityRollNumber(totalTech, target) then
                ply:LagCompensation(false)
                return
            end
            ...
            if playerStr >= 4 then targetEffects["baseDamage"] = 65 end
            ...
            if playerChr >= 4 then critBonus = critBonus + 10 end
