...
SWEP.PrintName = "Nuclear Missile"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Hits 3-5 times on random foes. 26 nuke damage for the first hit, but every subsequent hit deals 5 less damage than the last. If hitting an enemy inflicted by Burn/Freeze/Shock/Confusion, 5% bonus crit chance per hit. \n[CHR 6] 28 nuke damage for first hit instead. \n[DEX 6] Every subsequent hit deals 4 less damage instead."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if next(targetedTargets) == nil then
            ...
            local loopIterations = math.random(2, 5)

            local playerChr = tonumber(ply:GetNWInt("TBCCHR", 0))
            local playerDex = tonumber(ply:GetNWInt("TBCDEX", 0))

            self.DamageAmount = 26
            if playerChr >= 6 then self.DamageAmount = 28 end

            local lowerDamage = 5
            if playerDex >= 6 then lowerDamage = 4 end
