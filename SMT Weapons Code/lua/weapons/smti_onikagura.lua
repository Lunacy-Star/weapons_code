...
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 60 blade damage (all enemies). 15% chance to inflict Rage on hit. \n[STR 6] HP Cost reduced to 20 HP. \n[CHR 6] 20% chance to inflict Rage on hit"
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if playerStr >= 6 then hpCost = 20 end
        ...
        if next(targetedTargets) == nil then
            ...
            for _, ent in pairs(targetedTargets) do
                ...
                targetEffects["ailmentChance"] = 15
                ...
                if math.random(1, 100) <= targetEffects["ailmentChance"] then
                    ...
                    AssignStat(targetEffects["target"], "Rage",
                               debuffsTable["Rage"], "debuffs")

                    self:AnnounceMessage(
                        targetEffects["target"]:Name() .. " is now Enraged!")

                    HandleStatus(targetEffects["ply"], userBuffsTable,
                                 "ailmentReaction", "Rage", targetEffects)
                end
