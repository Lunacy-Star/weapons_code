...
SWEP.PrintName = "Cross Fortune"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Jun must have a Wind-Type and Light-Type skill to be able to use this ability (Persona skills applicable). Jun hits all enemies twice, dealing 30 Wind damage in the first hit and 30 Light damage in the second. The First hit has a (Luck + CHR)% to cause a random ailment."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        if windFound and lightFound then
            ...
            if next(targetedTargets) == nil then
                ...
                for _, ent in pairs(targetedTargets) do
                    ...
                    for i = 1, 2 do
                        ...
                        local ailmentChance = math.ceil(playerLuck + playerChr)
                        ...
                        if i == 1 then
                            if math.random(1, 100) <= ailmentChance then
                                HandleRandomAilment(targetEffects["ply"],
                                                    targetEffects["target"],
                                                    targetEffects)
                            end
                        end
            ...
            ply:ChatPrint(
                "You can't use this skill without a Wind and Light skill!")
        end
    end
