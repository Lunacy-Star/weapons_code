...
SWEP.PrintName = "Voltaic Ring"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "HP Cost: 4% Max HP.\nDeal 65 physical damage with Pierce on a single target. If the target is holding a skill or weapon that deals Elec damage, deals 75 physical damage instead. Heal Nekomata by 15% of damage dealt."
SWEP.Instructions = ""
...
SWEP.ElecHolderDamageAmount = 75
SWEP.HPCostPercent = 0.04
SWEP.Tech = 0
SWEP.Affinity = "Physical"
SWEP.Pierce = true
SWEP.HealPercent = 0.15
SWEP.Targets = "single"
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            local attackerMaxHP = ply:GetNWInt("TBCMAXHP", 100)
            local hpCost = math.ceil(attackerMaxHP * self.HPCostPercent)

            hpCost =
                hpCost +
                (HandleStatus(ply, userDebuffsTable, "increaseHPCost", hpCost) -
                    HandleStatus(ply, userBuffsTable, "decreaseHPCost", hpCost))
            ...
            local hasElecHolding = false
            for _, weapon in ipairs(target:GetWeapons()) do
                if IsValid(weapon) and weapon.Affinity == "Elec" then
                    hasElecHolding = true
                    break
                end
            end

            targetEffects["baseDamage"] = hasElecHolding and self.ElecHolderDamageAmount or self.DamageAmount
            ...
            -- Pierce: ignores Resist/Block/Drain/Repel entirely. Weakness still applies.
            ...
            local healAmount = math.ceil(targetEffects["baseDamage"] * self.HealPercent)
            local healerCurrentHP = ply:GetNWInt("TBCHP", 100)
            local healerMaxHP = ply:GetNWInt("TBCMAXHP", 100)
            ply:SetNWInt("TBCHP", math.min(healerCurrentHP + healAmount, healerMaxHP))
