...
SWEP.PrintName = "Locked-On"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 10 Dark damage to a single target. The following conditions trigger based on percent value and add onto each other: Weak to Dark: 5% of Naoto's Max HP Damage Marked with Cursed: 10% of Naoto's Max HP Damage."
SWEP.Instructions = ""
        ...
        if SERVER and IsValid(target) then
            ...
            if targetEffects["state"] == 'weak' then
                targetEffects["baseDamage"] =
                    targetEffects["baseDamage"] + (maxHP * 0.05)
            end
            if targetEffects["targetDebuffsTable"]["Cursed"] then
                targetEffects["baseDamage"] =
                    targetEffects["baseDamage"] + (maxHP * 0.1)
            end
