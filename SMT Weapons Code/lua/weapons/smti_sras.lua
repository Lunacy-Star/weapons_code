...
SWEP.Contact = ""
SWEP.Purpose =
    "A shotgun that allows users to switch between semiautomatic and pump action functions. +2 Luck."
SWEP.Instructions = ""
SWEP.Category = "SMT Physical Ranged Weapons"
SWEP.SpawnMenuIcon = "materials/entities/smti_sras.png"
...
function SWEP:Equip(ply)
    if SERVER then
        timer.Create(ply:Name() .. self.PrintName, 0.2, 1, function()
            local playerLuck = ply:GetNWInt("TBCLuck", 10)
            ply:SetNWInt("TBCLuck", playerLuck + 2)
        end)
    end
end
