...
SWEP.PrintName = "Dragon Kick"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Deal 50 Physical Damage to one enemy, apply 1 Tarukaja to self and 1 Rakunda to Enemy."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if targetEffects["userBuffsTable"]["Tarukaja"] then
                targetEffects["userBuffsTable"]["Tarukaja"].stacks = math.min(
                                                                         targetEffects["userBuffsTable"]["Tarukaja"]
                                                                             .stacks +
                                                                             1,
                                                                         4)
            else
                targetEffects["userBuffsTable"]["Tarukaja"] = {stacks = 1}
            end

            AssignStat(targetEffects["ply"], "Tarukaja",
                       targetEffects["userBuffsTable"]["Tarukaja"], "buffs")
            ...
            self:AnnounceMessage(targetEffects["ply"]:Name() .. " now has " ..
                                     targetEffects["userBuffsTable"]["Tarukaja"]
                                         .stacks .. " Tarukaja stacks!")
            self:AnnounceMessage(
                targetEffects["target"]:Name() .. " now has " ..
                    targetEffects["targetDebuffsTable"]["Rakunda"].stacks ..
                    " Rakunda stacks!")
