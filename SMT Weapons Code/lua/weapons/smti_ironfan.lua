...
SWEP.PrintName = "Iron Fan"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A fan crafted from the strips of metal to make it effective as a weapon. The sight of someone attacking with the fan is like watching a deadly yet beautiful dance. Every Sukukaja on the wielder buffs the damage by 5."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if not self:AbilityRollNumber(25, target) then
                ply:LagCompensation(false)
                return
            end
            ...
            if userBuffsTable["Sukukaja"] then
                targetEffects["baseDamage"] =
                    targetEffects["baseDamage"] +
                        (5 * userBuffsTable["Sukukaja"]["stacks"])
            end
