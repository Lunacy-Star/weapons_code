...
SWEP.PrintName = "Aura Shot"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A shotgun that converts its wielder's mental powers into bullets which it fires. Reduces Max MP by 10. Requires 20 MP to fire. \n[CHR 4] Requires 15 MP to fire instead."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if playerChr >= 4 then mpCost = 15 end
            ...
            if attackerMP >= mpCost then
                ...
                ply:ChatPrint("Not enough MP to use this gun.")
                ply:LagCompensation(false)
                return
            end
    ...
    if SERVER then
        timer.Create(ply:Name() .. self.PrintName, 0.2, 1, function()
            ...
            if currentMP >= maxMP then
                ply:SetNWInt("TBCMP", maxMP - 10)
            end
            ply:SetNWInt("TBCMAXMP", maxMP - 10)
        end)
    end
end
...
function SWEP:DropFunction()
    if SERVER and IsValid(self.Owner) then
        ...
        if currentMP >= maxMP then
            self.Owner:SetNWInt("TBCMP", maxMP + 10)
        end
        self.Owner:SetNWInt("TBCMAXMP", maxMP + 10)
    end
end
