...
SWEP.PrintName = "Star of Goetia"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "MP Cost: 90.\nDeal 40 Ruin damage to all enemies. If an enemy is asleep, deal 150 Ruin damage on that enemy instead and cure enemy's Sleep status. For all enemies cured of Sleep, gain +1 Tarukaja."
SWEP.Instructions = ""
    ...
    if SERVER then
        ...
        if next(targetedTargets) == nil then
            ...
            for _, ent in pairs(targetedTargets) do
                ...
                local wasAsleep = targetDebuffsTable["Sleep"] ~= nil

                targetEffects["baseDamage"] = wasAsleep and
                                                  self.SleepDamageAmount or
                                                  self.DamageAmount
                ...
                if wasAsleep and newHP > 0 then
                    RemoveStat(target, "Sleep", "debuffs")
                    sleepCuredCount = sleepCuredCount + 1
                    self:AnnounceMessage(target:Name() ..
                                             " is cured of Sleep by Star of Goetia!")
                end
        ...
        if sleepCuredCount > 0 then
            ...
            local currentStacks = (finalUserBuffsTable["Tarukaja"] and
                                      ...
                                      0
            local newStacks = math.min(currentStacks + sleepCuredCount, 4)

            AssignStat(ply, "Tarukaja", {stacks = newStacks}, "buffs")

            self:AnnounceMessage(
                ply:Name() .. " gains Tarukaja from Star of Goetia! (" ..
                    newStacks .. " stacks)"
            )
        end
