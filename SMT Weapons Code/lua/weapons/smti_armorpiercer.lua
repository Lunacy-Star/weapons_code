...
SWEP.PrintName = "Armorpiercer"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "A giant lance that can pierce through even the toughest armor. It is said only a select few powerful soldiers were able to handle this weapon. Every Tarukaja on the wielder buffs crit damage by 2%."
SWEP.Instructions = ""
    ...
    if tr.Hit and CheckIfValidTBCEntity(tr.Entity) then
        ...
        if SERVER and IsValid(target) then
            ...
            if targetEffects["state"] == 'crit' and userBuffsTable["Tarukaja"] then
                targetEffects["baseDamage"] =
                    targetEffects["baseDamage"] *
                        (1 + (0.02 * userBuffsTable["Tarukaja"]["stacks"]))
            end
            ...
            local attackerMP = ply:GetNWInt("TBCMP", 0) -- attacker's current TBCMP
