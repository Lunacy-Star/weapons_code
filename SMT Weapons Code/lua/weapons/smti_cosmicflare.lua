...
SWEP.PrintName = "Cosmic Flare"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Caster deals 50 nuke damage to self but gains +1 Rakukaja after the attack and deals 130% of the damage they received on all enemy targets. If at least one enemy inflicted by Burn/Freeze/Shock/Confusion is hit, gain +1 Tarukaja. \n[CHR 7]  Gains +1 Rakukaja and +1 Sukukaja instead."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if next(targetedTargets) == nil then
            ...
            self:AnnounceMessage(ply:Name() .. " received Rakukaja!")
            ...
            if playerDex >= 7 then
                ...
                self:AnnounceMessage(ply:Name() .. " received Sukukaja!")
            end

            damageToMultiply = targetEffects["baseDamage"] * 1.30

            local afflictedResult = false
            for _, ent in pairs(targetedTargets) do
                ...
                local targetEffects = {}
                targetEffects["baseDamage"] = damageToMultiply
                targetEffects["Affinity"] = self.Affinity
                ...
                if targetEffects["targetDebuffsTable"]["Burn"] or
                    targetEffects["targetDebuffsTable"]["Freeze"] or
                    targetEffects["targetDebuffsTable"]["Shock"] or
                    targetEffects["targetDebuffsTable"]["Confusion"] then
                    afflictedResult = true
                end
            ...
            if afflictedResult then
                ...
                self:AnnounceMessage(ply:Name() .. " received Tarukaja!")
            end
        end
