...
SWEP.PrintName = "Bloody Honeymoon"
SWEP.Author = "Nara"
SWEP.Contact = ""
SWEP.Purpose =
    "Eikichi must have a Ice-type skill and a Light-type skill to be able to use this ability (Persona skills applicable). Eikichi hits all enemies twice, dealing 30 Ice damage in the first hit and 30 Light damage in the second. The first hit has a chance of giving Eikichi 10 SP by (Luck + CHR)%."
SWEP.Instructions = ""
...
function SWEP:PrimaryAttack()
    ...
    for _, weapon in pairs(self.Owner:GetWeapons()) do
        if weapon.Affinity then
            if weapon.Affinity == "Ice" then iceFound = true end
            if weapon.Affinity == "Light" then lightFound = true end
        end
    end

    if SERVER then
        if iceFound and lightFound then
            ...
            if next(targetedTargets) == nil then
                ...
                for _, ent in pairs(targetedTargets) do
                    ...
                    for i = 1, 2 do
                        ...
                        local spRegenChance = math.ceil(playerLuck + playerChr)
                        ...
                        if i == 1 then
                            if math.random(1, 100) <= spRegenChance then

                                local plyMP =
                                    targetEffects["ply"]:GetNWInt("TBCMP", 0)
                                local maxMP =
                                    targetEffects["ply"]:GetNWInt("TBCMAXMP", 50)

                                plyMP = math.min(plyMP + 10, maxMP)

                                targetEffects["ply"]:SetNWInt("TBCMP", plyMP)
            ...
            ply:ChatPrint(
                "You can't use this skill without an Ice and Light skill!")
        end
    end
